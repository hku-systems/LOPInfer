...
def parse_kapoa_argv():
    ...
    sys.argv = sys.argv[:1] + ["--bbox", "--pose", "--imgsz", "640"]
...
class CompNavigation():
    ...
    def run_nms(self):
        ...
        rospy.loginfo(f"Work: {os.environ['work']}")
        rospy.loginfo(f"imgsz: {args.imgsz}")
        import torch
        ...
        while not rospy.is_shutdown():
            ...
            im0 = cv2.imread(os.path.join(os.environ["work"], "third_parties/kapao/res/crowdpose_100024.jpg"))
